...
module.exports = async function (context, req) {
    try {
        const serial = '0x' + randomBytes(10).toString('hex');
        const output = execSync(
            (env.OpenSSLPath ?? '') + 'openssl x509 -req -days 1826 -CA ca_cert.pem -CAkey ca_private_key.pem -extfile sign_device_cert_req.cfg -set_serial ' + serial,
            options = { input: req.rawBody }
        );
        context.res = {
            body: output
        }
    } catch (err) {
        context.log('OpenSSL invocation failed:\n' + err);
        context.res = {
            status: 500
        }
    }
